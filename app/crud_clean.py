...
def reset_daily_stats_if_needed(db: Session, pet: models.Pet):
    """
    Check if daily stats need to be reset (new day).
    If it's a new day, reset daily_exercise_seconds to 0.
    """
    now = datetime.now()
    today = now.date()
    
    # Check if last_reset_date is today
    if pet.last_reset_date:
        last_reset = pet.last_reset_date.date() if pet.last_reset_date.tzinfo else pet.last_reset_date.date()
        if last_reset >= today:
            # Already reset today, no need to reset
            return
...
def log_exercise(db: Session, user_id: str, log: schemas.ExerciseLogCreate):
    ...
    - Accumulate daily exercise time
    """
    ...
    reset_daily_stats_if_needed(db, pet)
...
def complete_quest(db: Session, user_id: str, user_quest_id: int):
    ...
    db.commit()
    return {"success": True, "pet": result["pet"], "breakthrough_required": result.get("breakthrough_required", False)}
...
def get_leaderboard_by_level(db: Session, limit: int = 10):
    return db.query(models.Pet, models.User.id)\
             ...
             .limit(limit)\
             .all()f r o m   s q l a l c h e m y . o r m   i m p o r t   S e s s i o n 